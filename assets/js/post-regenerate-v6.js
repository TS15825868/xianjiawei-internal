(()=>{
  ...
  const BLOCK=[['公司資訊',['台興山產有限公司','統一編號','公司電話','公司地址']],['30cc錯誤名稱',['龜鹿飲30cc玻璃瓶','30cc／瓶','小玻璃瓶']],['療效宣稱',['治療','治癒','保證改善','療效','藥到病除']]];
  ...
  function guard(card){
    ...
    else{box.className='xjw-ok';let note='AI規格文字預檢通過；圖片仍需人工確認16項。';if(t.includes('30cc'))note+=' 30cc必須裸罐無貼紙無外包裝。';if(t.includes('龜鹿湯塊'))note+=' 龜鹿湯塊只能75g深藍盒。';if(t.includes('龜鹿膏'))note+=' 龜鹿膏只用新版米白標籤。';if(['颱風','寒流','高溫','空氣品質'].some(x=>t.includes(x)))note+=' 即時資訊發布前需重新確認。';box.textContent=note}
  }
