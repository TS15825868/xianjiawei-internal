(()=>{
  ...
  async function ensureRaster(id){if(processing.has(id))return processing.get(id);const task=(async()=>{const post=await getPost(id);if(!isSvg(post.image_url))return{changed:false,post};toast('正在把候選圖轉成可審核 JPEG…');const blob=await renderSvgToJpeg(post.image_url);const uploaded=await upload(blob,id);const patched=await patchPost(id,{image_url:uploaded.url,image_width:uploaded.width||TARGET_SIZE,image_height:uploaded.height||TARGET_SIZE,image_bytes:uploaded.bytes||blob.size,image_source:rasterSourceLabel(post),image_quality_status:'ok'});toast('候選圖已轉成 JPEG；圖片已變更，原核准同步失效，請重新完成16項審核。');return{changed:true,post:patched,uploaded,previousStatus:post.status}})().finally(()=>processing.delete(id));processing.set(id,task);return task}
  document.addEventListener('click',async event=>{const button=event.target.closest('[data-post-publish-now], [data-post-status="approved"], [data-manual-package]');if(!button||button.dataset.xjwRasterReady==='1')return;const id=postIdFromButton(button);if(!id)return;event.preventDefault();event.stopImmediatePropagation();button.disabled=true;try{const result=await ensureRaster(id);button.disabled=false;if(!result.changed){button.dataset.xjwRasterReady='1';button.click();return}const approving=button.matches('[data-post-status="approved"]');if(approving){button.dataset.xjwRasterReady='1';button.click();return}toast('圖片已轉成正式JPEG並退回草稿；請重新完成16項圖文審核後再發布。');document.querySelector('[data-refresh]')?.click()}catch(error){button.disabled=false;toast(error.message||String(error),true)}},true);
  window.XJWExactOriginalRasterizer={version:'2026-08-09-v7-raster-invalidates-review',ensureRaster,isSvg,isDataSvg,rawUrl,rasterSourceLabel,approvedSources};
})();
