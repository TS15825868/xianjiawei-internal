(()=>{
  ...
  const LEGACY_PRODUCT_MAP={
    'guilu-gao.jpg':'guilu-gao.jpeg',
    'guilu-drink-30.jpg':'guilu-drink-30.jpeg',
    'guilu-drink-180.jpg':'guilu-drink-180.jpeg',
    'guilu-tangkuai.jpg':'guilu-tangkuai.jpeg',
    'guilu-jiao.jpg':'guilu-jiao-open-new.jpg',
    'luerong-fen.jpg':'luerong-fen.jpeg'
  };
  ...
  function actualProductSources(svg){
    let out=String(svg||'');
    for(const [oldName,newName] of Object.entries(LEGACY_PRODUCT_MAP)){
      out=out.replaceAll(`../../products-v3/${oldName}`,`${RAW_BASE}images/products-v2/${newName}`);
      out=out.replaceAll(`images/products-v3/${oldName}`,`${RAW_BASE}images/products-v2/${newName}`);
      out=out.replaceAll(`${PUBLIC_BASE}images/products-v3/${oldName}`,`${RAW_BASE}images/products-v2/${newName}`);
      out=out.replaceAll(`${RAW_BASE}images/products-v3/${oldName}`,`${RAW_BASE}images/products-v2/${newName}`);
    }
    out=out.replace(/href=(['"])(\.\.\/\.\.\/products-v2\/)/g,`href=$1${RAW_BASE}images/products-v2/`);
    out=out.replace(/href=(['"])(images\/products-v2\/)/g,`href=$1${RAW_BASE}$2`);
    out=out.replaceAll(PUBLIC_BASE+'images/products-v2/',RAW_BASE+'images/products-v2/');
    out=out.replaceAll(PUBLIC_BASE+'images/brand/line-oa/',RAW_BASE+'images/brand/line-oa/');
    return out;
  }
  async function renderSvgToJpeg(value){
    let svg=actualProductSources(await sourceSvg(value));
  ...
  function rasterSourceLabel(post){const current=String(post?.image_source||'').trim();if(current.startsWith('公開發布中心:'))return `${current}｜products-v2實際產品照片候選轉JPEG`;if(current)return `${current}｜products-v2實際產品照片候選轉JPEG`;return '仙加味候選｜products-v2實際產品照片候選轉JPEG'}
  ...
  window.XJWExactOriginalRasterizer={version:'2026-08-08-v5-products-v2',ensureRaster,isSvg,isDataSvg,rawUrl,rasterSourceLabel,actualProductSources};
})();