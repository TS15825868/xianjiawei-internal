...
const dm=(id,productId,file,keywords,alt,extra={})=>Object.freeze({id,productId,keywords,public_url:`${SITE}/images/dm-approved-v20260810/${file}?v=20260812-dm-corrected-v1`,binary_ready:true,approved:true,source:'corrected-detailed-dm',role:'detailed-dm',alt,...extra});
const CONFIG=Object.freeze({
  runtime:`formal-media-policy-${VERSION}`,
  approvalBatch:VERSION,
  productAuthority:'six-user-confirmed-product-visuals-valid-webp-with-products-v3-identity-reference',
  ...
  formalDmMedia:Object.freeze([
    dm('guilu-gao-dm','guilu-gao','guilu-gao-100g.webp',['龜鹿膏'],'仙加味龜鹿膏100g詳細DM'),
  ...
  hardReject:Object.freeze(['products-v2','products-v4-final-as-customer-main','ai-redrawn-wrong-product','wrong-30cc-container','wrong-180cc-package','collage','cropped-character','stretched-product','copy-image-mismatch','dm-copy-conflict','piece-weight-on-main-image','invalid-webp-binary'])
});
...
function installNote(catalog){const host=document.querySelector('.publish-separation-note');if(!host||host.querySelector('[data-formal-media-policy-note]'))return;const note=document.createElement('p');note.dataset.formalMediaPolicyNote='true';note.innerHTML='<strong>目前配圖規則：</strong>一般產品貼文固定使用六張正式產品圖；明確「DM／詳細DM」才使用修正版DM；試喝文固定使用獨立試喝主圖。對外WebP必須可正常解碼。龜鹿湯塊與龜鹿膠的每塊約重量只留詳細資料。任何換圖、重生成或改文案後一律回待審核並重新完成16項。';host.appendChild(document.createElement('br'));host.appendChild(note);}
window.addEventListener('DOMContentLoaded',()=>{runtime.loadLatestZipCatalog().then(installNote).catch(()=>installNote(null));});
})();
