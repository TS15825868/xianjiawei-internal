...
const url=path=>`${SITE}${path}?v=${VERSION}`;
const product=(id,path,keywords,alt,extra={})=>Object.freeze({id,keywords,public_url:url(path),binary_ready:true,approved:true,source:'current-user-approved-product-display-20260814',role:'official-product-image',alt,...extra});
const dm=(id,productId,path,keywords,alt,extra={})=>Object.freeze({id,productId,keywords,public_url:url(path),binary_ready:true,approved:true,source:'current-approved-detailed-dm-20260814',role:'detailed-dm',alt,...extra});
const CONFIG=Object.freeze({
  runtime:`formal-media-policy-${VERSION}`,
  approvalBatch:VERSION,
  productAuthority:'current-approved-customer-display-with-products-v3-identity-reference-only',
  displayPolicy:'一般產品貼文固定使用目前核准的顧客正式產品圖；只有明確DM／詳細DM意圖才使用獨立詳細DM；試喝貼文固定使用20260814最新核准小老闆試喝海報。三種角色不可互相誤代；products-v3只作實物身份、包裝與比例校正，不直接當目前一般貼文主圖。',
  ...
  trialMedia:Object.freeze({
    id:'trial',keywords:['試喝','3罐免費','試喝組','先試喝'],
    public_url:url('/images/trial/trial-poster-small-boss-official-v20260814.jpg'),
    binary_ready:true,approved:true,source:'current-user-approved-small-boss-trial-poster-20260814',role:'fixed-full-trial-master-only',
    alt:'仙加味龜鹿飲試喝組｜先試喝，再決定｜小老闆正式試喝主圖',
    validation:'do-not-regenerate-do-not-use-retired-trial'
  }),
  ...
  formalDmMedia:Object.freeze([
    ...
    dm('guilu-drink-180-dm','guilu-drink-180','/images/dm-approved-v20260810/guilu-drink-180cc.webp',['龜鹿飲180cc','180cc','鋁袋'],'仙加味龜鹿飲180cc鋁袋詳細DM'),
    dm('guilu-tangkuai-dm','guilu-tangkuai','/images/dm-approved-v20260810/guilu-tangkuai-75g.webp',['龜鹿湯塊','湯塊75g','湯塊'],'仙加味龜鹿湯塊75g／盒｜8塊裝詳細DM',{validation:'no-piece-weight-on-main-spec'}),
    dm('guilu-jiao-dm','guilu-jiao','/images/dm-approved-v20260810/guilu-jiao-600g.webp',['龜鹿膠','600g','1斤','32塊'],'仙加味龜鹿膠600g（1斤）／盒｜32塊裝詳細DM',{validation:'no-piece-weight-on-main-spec'}),
    dm('luerong-fen-dm','luerong-fen','/images/dm-approved-v20260810/lurong-fen-75g.webp',['鹿茸粉','鹿茸','75g'],'仙加味鹿茸粉75g詳細DM')
  ]),
  ...
  regeneration:Object.freeze({invalidateApproval:true,invalidateSchedule:true,returnStatus:'pending_review',autoApprove:false,autoSchedule:false,autoPublish:false,reviewItems:16,actualProductReference:'products-v3'}),
  hardReject:Object.freeze([
    'products-v2','products-v4-final-as-customer-main','ai-redrawn-wrong-product','wrong-30cc-container','wrong-180cc-package','collage','cropped-character','stretched-product','copy-image-mismatch','dm-copy-conflict','piece-weight-on-main-image','invalid-image-binary','guilu-gao-100g.webp','/trial.webp','trial-clean-v4.svg','trial-small-boss.webp','/images/dm-approved-v20260810/guilu-drink-30cc.webp'
  ])
});
const normalize=s=>String(s||'').toLowerCase();
const rejected=c=>c?.rejected===true||c?.status==='rejected'||c?.product_redrawn===true||c?.status==='not_zip_authority'||/guilu-gao-100g\.webp|\/trial\.webp|trial-clean-v4\.svg|trial-small-boss\.webp|\/images\/dm-approved-v20260810\/guilu-drink-30cc\.webp/i.test(String(c?.public_url||''));
...
const scoreCandidate=(copy,candidate)=>{const c=candidate||{};if(rejected(c))return-Infinity;const semantic=semanticScore(copy,c);if(semantic<=0)return-Infinity;let priority=0;const source=normalize(c.source||c.origin||c.source_file||'');if(source.includes('trial-poster-20260814'))priority+=360;else if(source.includes('current-approved-detailed-dm'))priority+=300;else if(source.includes('current-user-approved-product-display'))priority+=280;else if(source.includes('formal-media'))priority+=240;else if(source.includes('zip')||source.includes('user')||c.source_file)priority+=100;if(c.approved===true||c.status==='approved')priority+=60;return semantic+priority;};
...
const mediaForProduct=id=>CONFIG.formalProductMedia.find(item=>item.id===String(id||''))||null;
const dmForProduct=id=>CONFIG.formalDmMedia.find(item=>item.productId===String(id||''))||null;
...
const mediaForTopic=topic=>{const item=topic||{};if(item.imageMode==='official_trial')return CONFIG.trialMedia;if(item.imageMode==='official_product'){const ids=Array.isArray(item.productIds)?item.productIds:[];if(ids.length===1)return mediaForProduct(ids[0]);}return null;};
const loadJson=async sourceUrl=>{const response=await fetch(sourceUrl,{cache:'no-store',credentials:'same-origin'});if(!response.ok)throw new Error(`media catalog ${response.status}`);return response.json();};
const runtime={...CONFIG,latestCatalog:null,semanticScore,chooseSourceCandidate:chooseSource,chooseBestCandidate:choosePublishable,mediaForProduct,dmForProduct,mediaForTopic,detectRole,resolveMedia,needsBinarySync:(copy,candidates)=>resolveMedia(copy,candidates).status==='needs_binary_sync',needsGeneration:(copy,candidates)=>resolveMedia(copy,candidates).status==='regenerate_if_missing'};
