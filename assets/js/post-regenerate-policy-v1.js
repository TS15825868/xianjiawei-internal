(()=>{
  'use strict';
  const VERSION='2026-08-09-single-system-generation-policy-v2-free-roundtrip';
  ...
  function postId(card){
    const button=card?.querySelector('[data-post-view],[data-post-edit]');
    return escText(button?.dataset?.postView||button?.dataset?.postEdit);
  }
  function promptFor(mode,card){
    ...
    if(mode==='all') return `請把這篇仙加味社群貼文的「文案＋圖片」整套重新製作。先重寫文案，再直接使用圖像生成能力產出一張待審核主圖。舊圖不要沿用，也不要做拼貼。${common}\n\n圖片必須是1:1單一完整場景；若需要產品，只能使用products-v3正式實拍等比例融入同一場景；若無法安全合成正式實拍，改生成不含產品本體的完整情境圖並預留乾淨位置，絕對不要AI重畫產品。完成後仍是待審核。${back}`;
    return `請直接使用圖像生成能力，為這篇仙加味社群貼文重新製作一張「待審核候選主圖」。不要只回提示詞或製圖步驟。${common}\n\n先判斷文案是否需要產品；不需要就不要硬塞。需要產品時只能使用products-v3正式實拍完整等比例融入單一場景。禁止任何拼貼式構圖。若無法安全使用正式產品原圖，就生成不含產品的完整情境圖並預留乾淨位置，不能虛構產品。輸出1:1社群主圖，完成後仍須16項人工審核。${back}`;
  }
  function copy(text){
  ...
  function remember(id,mode){
    try{localStorage.setItem(key(id),JSON.stringify({mode,startedAt:new Date().toISOString(),version:VERSION}));}catch{}
  }
  ...
  async function jsonRequest(path,body){
    const response=await nativeFetch(path,{method:'POST',credentials:'same-origin',cache:'no-store',headers:{'content-type':'application/json'},body:JSON.stringify(body||{})});
    const raw=await response.text();let data={};try{data=raw?JSON.parse(raw):{}}catch{}
    if(!response.ok)throw new Error(data.error||`系統操作失敗（HTTP ${response.status}）`);
    return data;
  }
  async function beginRegeneration(id,mode){
    const data=await jsonRequest(`/api/posts/${encodeURIComponent(id)}/regeneration-start`,{mode});
    remember(id,mode);
    return data;
  }
  async function markReady(id,mode){
    const data=await jsonRequest(`/api/posts/${encodeURIComponent(id)}/regeneration-ready`,{mode});
    forget(id);
    document.dispatchEvent(new CustomEvent('xjw-regeneration-ready',{detail:{id,mode,data}}));
    toast('新文案／圖片已自動送回「待審核」，請重新完成16項圖文審核。');
    return data;
  }
  async function launch(mode,card){
    ...
    toast('舊核准與排程已撤銷；正式生成指令已複製，正在開啟 ChatGPT。生成後回此卡按「生成後回填／上傳」。');
    window.open(`https://chatgpt.com/?q=${encodeURIComponent(prompt)}`,'_blank','noopener');
  }
  async function openReturnEditor(card){
    const id=postId(card);if(!id)throw new Error('找不到貼文 ID，請重新整理後再試。');
    const marker=recalled(id);const mode=['image','copy','all'].includes(marker?.mode)?marker.mode:'all';
    await beginRegeneration(id,mode);
    const edit=card.querySelector(`[data-post-edit="${CSS.escape(id)}"]`)||card.querySelector('[data-post-edit]');
    if(!edit)throw new Error('目前沒有可編輯入口，請重新整理後再試。');
    toast('請貼上新文案或使用「從手機／平板／電腦上傳圖片」。儲存後會自動進入待審核。');
    edit.click();
  }
  function enhanceReturnButtons(root=document){
    root.querySelectorAll('.xjw-row,.publish-card').forEach(card=>{
      if(card.dataset.status==='published'||card.dataset.locked==='true')return;
      const id=postId(card),actions=card.querySelector('.xjw-actions');if(!id||!actions||actions.querySelector('[data-xjw-regeneration-return]'))return;
      const button=document.createElement('button');button.type='button';button.className='btn small green';button.dataset.xjwRegenerationReturn=id;button.textContent=recalled(id)?'生成後回填／上傳｜待送審':'回填新文案／圖片';
  ...
  window.fetch=async function xjwGenerationAwareFetch(input,init={}){
    ...
    try{
      ...
      if(response.ok&&match&&['PUT','PATCH'].includes(method)){
        ...
        if(marker){
          try{await markReady(id,marker.mode||'all')}catch(error){toast(`貼文已儲存，但自動送回待審核失敗：${error.message||error}。請保留此頁並再按一次「生成後回填／上傳」。`,true)}
        }
      }
    }catch{}
    return response;
  ...
  document.addEventListener('click',event=>{
    ...
    if(returnButton){
      const card=returnButton.closest('.xjw-row,.publish-card');if(!card)return;
      event.preventDefault();event.stopPropagation();event.stopImmediatePropagation();
      openReturnEditor(card).catch(error=>toast(error?.message||String(error),true));
      return;
    }
  ...
  const observer=new MutationObserver(mutations=>{for(const mutation of mutations)for(const node of mutation.addedNodes)if(node?.nodeType===1)enhanceReturnButtons(node.matches?.('.xjw-row,.publish-card')?node:node)});
  ...
  document.addEventListener('xjw-publishing-list-rendered',()=>enhanceReturnButtons());
  window.XJWGenerationPolicy=Object.freeze({version:VERSION,officialProducts:OFFICIAL,hardRules:HARD_RULES,promptFor,beginRegeneration,markReady});
})();
