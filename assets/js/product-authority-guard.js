(()=>{
  ...
  window.XJW_PRODUCT_AUTHORITY=Object.freeze({
    version:'current-product-authority-20260814-v2',
    products:PRODUCTS,
    byId:BY_ID,
  ...
  function contentErrors(text=''){
    ...
    for(const segment of productSegments(value,'龜鹿膠'))if(/每塊約?\s*18\.75\s*g/i.test(segment))errors.push('龜鹿膠每塊約18.75g只留產品詳細／內部資料，不放對外貼文、產品圖或DM主規格。');
  ...
  function insertAuthorityNote(form){
    ...
    note.textContent='目前正式主規格：龜鹿膏100g／罐；30cc／罐（小玻璃罐）；180cc／包（鋁袋）；龜鹿湯塊75g／盒｜8塊裝；龜鹿膠600g（1斤）／盒｜32塊裝；鹿茸粉75g／罐。所有產品使用時間依個人使用習慣與作息時間安排；30cc為每日 1-2罐；180cc保留每日一包；龜鹿飲不設定固定白天時段。一般顧客產品圖使用customer-display；詳細DM、試喝海報與products-v3身份／比例參考分開。未知尺寸不猜、不拉伸。';
    grid.prepend(note);
  }
