(()=>{
  ...
  const PRODUCTS=Object.freeze([
    ...
    {id:'guilu-drink-30',name:'龜鹿飲30cc玻璃罐',spec:'30cc／罐（小玻璃罐）',allowedSpecs:['30cc／罐（小玻璃罐）'],ingredients:['水','龜板萃取物','鹿角萃取物','粉光蔘','枸杞','紅棗','黃耆'],usagePrimary:'每日 1–2 罐；飲用時間可依個人使用習慣與作息時間安排',image:image('images/customer-display-v20260812/guilu-drink-30cc.avif'),identity:image('images/products-v3/guilu-drink-30.jpg'),dimensions:{diameterMm:42,heightMm:51},scaleRule:'必須維持小玻璃裸罐原罐型與比例；無貼紙，不得稱瓶、做高、做胖或放大。'},
  ...
  window.XJW_PRODUCT_AUTHORITY=Object.freeze({
    version:'current-product-authority-20260820-v5-public-master',
    source:'public-product-master.json',
    sourceAuthority:'user-confirmed-current',
    products:PRODUCTS,
    byId:BY_ID,
    ...
    guiluDrink30UsagePrimary:'每日 1–2 罐；飲用時間可依個人使用習慣與作息時間安排',
  ...
  function toast(message,error=true){
    ...
    if(!root){ if(typeof alert==='function')alert(message); return; }
  ...
  function contentErrors(text=''){
    ...
    for(const segment of productSegments(value,'龜鹿飲30cc玻璃罐')){
      if(/(每日一罐|每日\s*1\s*罐)/.test(segment))errors.push('龜鹿飲30cc目前使用方式為「每日 1–2 罐」；飲用時間可依個人使用習慣與作息時間安排。');
    }
    ...
    for(const segment of productSegments(value,'龜鹿膏')){
    ...
    for(const segment of productSegments(value,'龜鹿湯塊')){
      if(/(300\s*g|600\s*g)/i.test(segment))errors.push('龜鹿湯塊目前正式主規格只有「75g （2兩）／盒｜8塊裝」。');
    }
  ...
  function insertAuthorityNote(form){
    if(!form||form.querySelector('[data-product-authority-note]'))return;
    const grid=form.querySelector('.form-grid');
    if(!grid)return;
    const note=document.createElement('div');
    note.dataset.productAuthorityNote='1';
    note.className='xjw-ok';
    note.textContent='目前官網公開六項：龜鹿膏100g／罐；龜鹿飲30cc／罐（小玻璃罐）；龜鹿飲180cc／包（鋁袋）；龜鹿湯塊75g （2兩）／盒｜8塊裝，每塊約9.375g；龜鹿膠600g （1斤）／盒｜32塊裝，每塊約18.75g；鹿茸粉75g／罐。龜鹿膏使用時間依個人習慣與作息；30cc每日 1–2 罐；180cc每日一包。產品圖、詳細DM、試喝海報與products-v3身份參考分開；未知尺寸不猜、不拉伸。';
    grid.prepend(note);
  }
  ...
  function enforceProductImageDisplay(root=document){
    root.querySelectorAll?.('img[data-product-id],img[data-product]').forEach(img=>{
      ...
      img.dataset.xjwMediaRole='customer-product-display';
    });
  }
  function refresh(){
    const productForm=document.getElementById('recordForm');
    if(productForm&&location.hash==='#products')insertAuthorityNote(productForm);
    enforceProductImageDisplay(document);
  }
  ...
  document.addEventListener('submit',event=>{
    ...
    toast(`目前正式產品資料檢查未通過：${errors.join('；')}`);
  },true);
