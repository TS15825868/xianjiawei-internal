(()=>{
  ...
  function applySafeMode(){
    ...
    document.documentElement.dataset.publishingPublishReady=publishReady?'true':'false';
    ...
    document.querySelectorAll(PUBLISH_SELECTOR).forEach(button=>{
      if(!safeMode&&!publishReady){
        if(!button.disabled)button.dataset.xjwPlatformDisabled='1';
        button.disabled=true;
        button.title=platformChecked?'已設定的平台 API 尚未全部通過健康檢查，正式發布暫停。':'平台 API 正在背景安全檢查，確認後自動開放正式發布。';
      }else if(button.dataset.xjwPlatformDisabled==='1'&&!button.dataset.xjwOfflineDisabled){
        delete button.dataset.xjwPlatformDisabled;
        button.disabled=false;
        if(/平台 API/.test(button.title||''))button.removeAttribute('title');
      }
    });
    ...
    if(state&&safeMode&&document.documentElement.dataset.publishingOffline!=='true'){
      ...
      state.classList.remove('safe-mode');
      if(platformChecked&&!publishReady&&document.documentElement.dataset.publishingOffline!=='true')state.textContent='核心正常｜平台發布暫停';
    }
  }
    ...
    running=(async()=>{
      ...
      try{
        ...
        if(full){
          ...
          publishReady=Array.isArray(report.blockingPlatformFailures)?report.blockingPlatformFailures.length===0:true;
        }
      }catch(error){
        if(full&&report?.ok){platformChecked=true;publishReady=false;report={...report,platformProbeError:String(error?.message||error)}}
        else{report={ok:false,worker:{ok:navigator.onLine!==false},d1:{ok:false,error:String(error?.message||error)},access:{ok:false},login:{ok:false},safeMode:true};safeMode=true;publishReady=false}
      }
      render();applySafeMode();
      document.dispatchEvent(new CustomEvent('xjw-publishing-readiness',{detail:{report,safeMode,publishReady,platformChecked,version:VERSION}}));
      return report;
