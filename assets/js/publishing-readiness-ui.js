(()=>{
  'use strict';
  const VERSION='20260914-publishing-readiness-ui-v9-live-summary-lock';
  ...
  let safeMode=true,publishReady=false,platformChecked=false,report=null,running=null,lastFullProbe=0,platformSummaryObserver=null,renderingPlatformSummary=false;
  ...
  function statusChip(label,item){
    ...
    const cls=manual||degraded||unconfigured?'manual':ok?'ok':'bad';
  ...
  function renderPlatformSummary(probe){
    ...
    const html=platformSummaryHtml(probe);if(!html)return;
    if(root.dataset.livePlatformSummary==='1'&&root.dataset.livePlatformSignature===html)return;
    renderingPlatformSummary=true;
    root.innerHTML=html;
    root.dataset.livePlatformSummary='1';
    root.dataset.livePlatformSignature=html;
    queueMicrotask(()=>{renderingPlatformSummary=false});
  }
  function ensurePlatformSummaryObserver(){
    const root=$('#platformSummary');
    if(!root||platformSummaryObserver)return;
    platformSummaryObserver=new MutationObserver(()=>{
      if(renderingPlatformSummary||!report?.platformProbe)return;
      const expected=platformSummaryHtml(report.platformProbe);
      if(expected&&root.innerHTML!==expected)renderPlatformSummary(report.platformProbe);
    });
    platformSummaryObserver.observe(root,{childList:true,subtree:true,characterData:true});
  }
  function render(){
    ...
    if(report.platformProbe){renderPlatformSummary(report.platformProbe);ensurePlatformSummaryObserver()}
  }
  ...
  async function run({full=false}={}){
    ...
    running=(async()=>{
      ...
      try{
        ...
        if(full){
          lastFullProbe=Date.now();platformChecked=true;
          const blocking=Array.isArray(report.blockingPlatformFailures)?report.blockingPlatformFailures:[];
          publishReady=!report.platformProbeError&&Boolean(report.platformProbe)&&blocking.length===0;
        }
      }
  ...
  document.addEventListener('xjw-publishing-list-rendered',()=>{applySafeMode();if(report?.platformProbe){renderPlatformSummary(report.platformProbe);ensurePlatformSummaryObserver()}});
  document.addEventListener('DOMContentLoaded',()=>{render();applySafeMode();ensurePlatformSummaryObserver();setTimeout(()=>run({full:false}).then(()=>schedulePlatformProbe(900)).catch(()=>{}),120)});
  ...
  window.XJWPublishingReadiness={version:VERSION,run:publicRun,runBlocking:run,getReport:()=>report,isSafeMode:()=>safeMode,isPublishReady:()=>publishReady,applySafeMode,schedulePlatformProbe,renderPlatformSummary};
})();
