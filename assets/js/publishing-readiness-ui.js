(()=>{
  ...
  function statusChip(label,item){const ok=item?.ok===true,manual=item?.mode==='manual',cls=ok?'ok':manual?'manual':'bad';const detail=ok?(item.latencyMs!=null?`${item.latencyMs}ms`:'正常'):manual?'人工':'異常';const reason=item?.error||item?.reason||(Array.isArray(item?.missing)&&item.missing.length?`缺少 ${item.missing.join('、')}`:'');return`<span class="readiness-chip ${cls}"${reason?` title="${esc(reason)}"`:''}><strong>${esc(label)}</strong><small>${esc(detail)}</small></span>`}
  ...
  function render(){
    ...
    for(const name of ['Facebook','Instagram','LINE OA','LINE VOOM','Google 商家'])if(p[name]){base.push(statusChip(name,p[name]));const problem=issue(name,p[name]);if(problem)issues.push(problem)}
  ...
  async function run({full=false}={}){
    ...
    running=(async()=>{
      ...
      try{const core=await timeoutFetch('/healthz/core',4500);if(!core.ok)throw new Error(`Worker HTTP ${core.status}`);const readiness=await timeoutFetch(`/healthz/readiness${full?'?probe=1':''}`,full?10000:6500);const parsed=await readJson(readiness);report=parsed.data||{};safeMode=!Boolean(report.ok);if(full){lastFullProbe=Date.now();platformChecked=true;publishReady=Array.isArray(report.blockingPlatformFailures)?report.blockingPlatformFailures.length===0:true}}
