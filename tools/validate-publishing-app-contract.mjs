...
if(!/publishing-app-v2\.js\?v=[^"']+/.test(html))throw new Error('publishing.html 的正式主程式缺少快取版本識別');
if(!/post-bank-sync\.js\?v=[^"']+/.test(html))throw new Error('publishing.html 的500篇母庫同步工具缺少快取版本識別');
if(!/post-regenerate-policy-v1\.js\?v=[^"']+/.test(html))throw new Error('publishing.html 的重生成流程缺少快取版本識別');
...
if(!/FAST_API_VERSION=['"][^'"]*fast-read[^'"]*['"]/.test(production))throw new Error('手機快速讀取契約缺少 fast-read 版本識別');
for(const token of [
...
console.log('PASS：獨立貼文系統以能力契約驗收：核心安全診斷、立即發布不受固定時段限制、單一重生成流程、500篇同步、快速Access驗證、D1安全模式、平台API預檢、server pagination、離線快取、LINE keep-warm與逐平台發布結果均存在；正確新版runtime不再因舊版號誤擋。');
