...
async function fetchMaster(){
  const response=await fetch(MASTER_URL,{headers:{'user-agent':'xianjiawei-internal-product-ssot'}});
  ...
  if(JSON.stringify(ids)!==JSON.stringify(EXPECTED_IDS))throw new Error(`產品母資料品項或順序錯誤：${ids.join(',')}`);
  return master;
}
...
async function assertSnapshot(master){
  ...
  if(snapshot.PRODUCT_MASTER_META?.authority!==master.authority)throw new Error('ERP 快照 authority 與母資料不同步');
  if(snapshot.PRODUCT_MASTER_META?.version!==master.version)throw new Error(`ERP 快照版本 ${snapshot.PRODUCT_MASTER_META?.version||'missing'} 與母資料 ${master.version} 不同步`);
  ...
  if(JSON.stringify(actual)!==JSON.stringify(expected))throw new Error('ERP 產品守門快照內容與唯一產品母資料不同步；請執行 npm run sync:product-master');
}
...
async function main(){
  ...
  if(process.argv.includes('--write')){
    ...
    console.log(`SYNCED ERP product snapshot <- ${master.version}`);
    return;
  }
  await assertSnapshot(master);
  console.log(`PASS ERP product SSOT ${master.version}`);
}
