...
must(publishing.includes('20260810-single-system-v3-true-originals'),'貼文系統沒有強制載入真正products-v3原圖版重生成流程');
must(publishing.includes('20260810-product-authority-v6-true-originals'),'貼文系統沒有強制載入真正products-v3原圖版產品守門');
...
for(const token of ['禁止拼貼','products-v3','AI絕對不得重畫','30cc','Ø42×H51mm','180cc','0.60～0.68','小老闆','不可裁切','16項','待審核','/regeneration-start','/regeneration-ready','20260810-products-v3-true-originals-v2']){
  must(policy.includes(token),`生成守門缺少硬規格／免費閉環／真正產品原圖版本：${token}`)
}
for(const token of ['20260810-products-v3-true-originals-v2','products-v3-true-original-product-photos','Ø42','H51','0.60','0.68']){
  must(productGuard.includes(token),`產品圖片守門沒有鎖定真正products-v3原圖或實際比例：${token}`)
}
...
console.log('PASS：正式部署只保留仙加味唯一貼文審核發佈系統；產品守門與重生成流程都鎖定真正products-v3原圖、禁止拼湊、產品不得AI重畫並要求16項重審。');
