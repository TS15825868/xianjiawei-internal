...
must(publishing.includes('zip-media-assistant.js'),'正式系統沒有載入唯一正式配圖助理');
...
must(/zip-media-assistant\.js\?v=[^"']+/.test(publishing),'正式配圖助理缺少正式快取版本識別');
...
for(const token of ['formalProductMedia','semanticScore','user_zip_approved','approved_existing','regenerate_if_missing','pending_review','reviewItems:16','ai-redrawn-product','copy-image-mismatch','needs_binary_sync','binary_ready','public_url','formal_product_media']){
...
for(const token of ['resolveMedia','approved_existing','needs_binary_sync','regenerate_if_missing','formal_product_media','套用正式圖','套用 ZIP 合格圖','xjw-publishing-list-rendered']){
  must(mediaAssistant.includes(token),`唯一正式配圖助理缺少能力：${token}`)
}
...
must(publishing.includes('最新貼文圖來源')&&publishing.includes('待同步原圖')&&publishing.includes('正式圖與 ZIP 都沒有合格來源'),'貼文中心顧客可見說明沒有同步正式圖／ZIP／待同步／重生成能力');
...
must(pkg.includes('zip-media-assistant.js'),'正式部署沒有包含唯一正式配圖助理');
