...
// Root must be the full internal operations console again.
...
// Posts/calendar are intentionally separated to one current publishing interface.
...
// Formal top-level Worker keeps full ERP plus current publishing safety.
...
// Current media roles and product facts.
...
for(const token of ['formalProductMedia','semanticScore','user_zip_approved','approved_existing','regenerate_if_missing','pending_review','reviewItems:16','needs_binary_sync','formal_product_media','latestZipCatalog'])must(mediaPolicy.includes(token),`最新媒體權威runtime缺少能力：${token}`);
...
// Current ZIP is capability-driven, never fixed to an old batch count.
...
// Build publishes both UIs, not two publishing implementations.
...
console.log(`PASS：完整ERP根介面與核心模組已恢復，貼文／排程仍只走單一獨立publishing.html；production-entry保留16項審核、立即發布與平台安全；產品／DM／試喝／products-v3身份參考角色分離，最新 ${latestZip.source}/${latestZip.candidate_count} 張候選採能力式驗收。`);
