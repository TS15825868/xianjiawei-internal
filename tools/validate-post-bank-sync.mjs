...
must(!html.includes('<script src="/assets/js/post-bank-sync.js'),'手機正式首屏不得同步載入母庫工具；母庫同步保留在部署包並由後端／Workflow處理');
...
must(hasAny([/儲存後(?:會)?自動回到[「"]?待審核/,/生成完成後.*待審核/,/重新生成後.*待審核/,/完成後.*重新進入待審核/]),'貼文系統沒有說明重生成完成後必須回待審核');
must(/不會自動核准.{0,20}發布/.test(normalized)||/不自動核准.{0,20}不自動發布/.test(normalized),'貼文系統沒有說明重生成不得自動核准／排程／發布');
...
must(html.includes('products-v3只作產品本體身份、包裝與比例校正')||html.includes('products-v3只作產品身份、包裝與比例校正'),'貼文中心必須把products-v3說明為身份／包裝／比例校正，而不是一般顧客主圖');
must((html.includes('一般產品貼文只用六張正式產品圖')||html.includes('一般產品貼文使用正式產品原圖'))&&html.includes('詳細DM')&&(html.includes('試喝文固定用目前核准')||html.includes('試喝圖各自使用核准素材')),'貼文中心必須清楚分離一般產品圖、詳細DM與試喝圖角色');

console.log('PASS：貼文母庫採目前能力驗收：張數動態、ID唯一、需重生成清舊圖、已發布／活動冷卻保護、不自動核准／排程／發布；母庫同步不再佔用iPhone首屏，正式產品圖、詳細DM、試喝圖與products-v3參考角色仍分離。');
