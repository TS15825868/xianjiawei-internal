...
must(html.includes('已知需重生成內容維持無舊錯圖狀態'),'貼文系統沒有向使用者說明重生成安全門檻');
...
console.log(`PASS：500篇母庫以可信postMessage來源重建；exporter與內部sync雙邊驗證退役資產已移除、products-v3正式原圖與至少${knownMinimum}篇已知重生成。正式內容依source id去重，只有無source id舊資料才以標題相容去重；已發布／活動冷卻不動，安全候選只進待審核，需重生成只建草稿，絕不自動發布；升級新版不再被舊v3/v5字串誤擋。`);
