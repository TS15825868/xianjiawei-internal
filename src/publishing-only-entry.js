...
const RETIRED_EXACT=new Set(['/api/overview','/api/settings','/api/users','/api/audit','/api/assets','/api/brand-content']);
...
function retired(path){return RETIRED_EXACT.has(path)||RETIRED_PREFIXES.some(prefix=>path.startsWith(prefix))}
...
export default{
 async fetch(request,env,ctx){
  ...
  if(retired(url.pathname))return json({error:'此功能目前已從正式系統移除。仙加味目前只保留貼文審核發佈系統。',code:'XJW_PUBLISHING_ONLY',publishing_path:'/publishing.html',version:VERSION},404);
  if(request.method==='GET'&&url.pathname===LATEST_POST_ZIP_MANIFEST&&env?.ASSETS?.fetch){const asset=await env.ASSETS.fetch(request);if(asset.ok){const headers=new Headers(asset.headers);headers.set('cache-control','no-store');headers.set('x-xianjiawei-publishing-only',VERSION);return new Response(asset.body,{status:asset.status,headers})}}
  const response=await app.fetch(request,env,ctx);
  if(request.method==='GET'&&['/healthz','/healthz/core'].includes(url.pathname)){
   try{const [body,media]=await Promise.all([response.clone().json(),currentMediaAuthority(request,env)]);return json({...body,uiRuntime:UI_RUNTIME,productImageVersion:PRODUCT_IMAGE_VERSION,productImageAuthority:'products-v3-latest-original-product-photos',postBankSyncVersion:POST_BANK_SYNC_VERSION,postBankValidation:'capability-based',postBankSizePolicy:'current-catalog-dynamic-no-fixed-count',formalMediaRuntime:FORMAL_MEDIA_RUNTIME,latestPostZipManifest:LATEST_POST_ZIP_MANIFEST,latestPostZipDynamic:true,latestPostZip:media.latestPostZip,latestPostZipCandidates:media.latestPostZipCandidates,formalMediaApprovalBatch:media.formalMediaApprovalBatch,latestPostZipBinaryStatus:media.latestPostZipBinaryStatus,postImagePriority:'user_zip_approved',formalMediaDecisionOnPostCard:true,singleMediaAssistant:true,semanticImageMatchRequired:true,formalProductMediaPreferred:true,regenerateOnlyIfNoApprovedMatch:true,zipSourceMatchCanWaitForBinarySync:true,reviewItemsAfterMediaChange:16,guardVersionPolicy:'current-authority-not-historical-version-pin',publishingOnly:true,publishingOnlyVersion:VERSION,retiredErpApisBlocked:true,...(url.pathname==='/healthz'?{publishingReviewGateVersion:REVIEW_GATE_VERSION,freeRegenerationRoundTrip:true,regenerationReturnsToPendingReview:true,regenerationStartEndpoint:'/api/posts/:id/regeneration-start',regenerationReadyEndpoint:'/api/posts/:id/regeneration-ready'}:{})},response.status)}catch{return response}
  }
  return response;
 },
