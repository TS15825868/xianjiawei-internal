...
const VERSION='2026-08-15-publishing-review-gate-v5-current-product-authority';
...
async function submitForReview(request,env,profile,id,before){
  if(!canRegenerate(profile))return json({error:'沒有送待審核的權限'},403);
  if(!before)return json({error:'找不到貼文'},404);
  if(clean(before.status)!=='draft')return json({error:'只有草稿可以送待審核'},409);
  if(!clean(before.copy)&&!clean(before.headline))return json({error:'貼文沒有文案，請先完成文案再送待審核'},409);
  if(!clean(before.image_url))return json({error:'貼文沒有圖片，請先補上圖片再送待審核'},409);
  ...
  const after=await postRow(env,id);
  await regenerationAudit(env,request,profile,'草稿送待審核',id,before,after,'');
  return json({ok:true,id,status:'pending_review',review_required:true,required_checks:REQUIRED_CHECKS,message:'已送待審核；必須完成人工16項圖文審核後才能核准。'});
}
